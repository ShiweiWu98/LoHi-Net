...
def _as_wavelet(wavelet):
    """Ensure wavelet is a pywt Wavelet object."""
...
def construct_2d_filt(lo, hi) -> torch.Tensor:
    """Construct 2D wavelet filters via outer products."""
    ll = _outer(lo, lo)
    lh = _outer(hi, lo)
    hl = _outer(lo, hi)
    hh = _outer(hi, hi)
...
def get_filter_tensors(
    ...
    """Convert pywt wavelet filters to torch tensors."""
...
def _get_pad(data_len: int, filt_len: int) -> Tuple[int, int]:
    """Compute padding for wavelet transform."""
    ...
    def forward(self, x):
        ...
        for _ in range(self.level):
            ...
            wavelet_component.append(
                (lh_component.squeeze(2), hl_component.squeeze(2), hh_component.squeeze(2))
            )
