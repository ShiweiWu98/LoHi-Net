...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if scan_type == 'hl':
            ...
            self.forward_core = self.forward_corehl
            # ************************************ hl weight *****************************************
            ...
            self.forward_core = self.forward_corelh
            # ************************************ lh weight *****************************************
            ...
            self.forward_core = self.forward_corehh
            # ************************************ hh weight *****************************************
        else:
            raise ValueError(f"Unsupported scan_type: {scan_type}. Must be 'hl', 'lh', or 'hh'.")
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # 根据 scan_type 调用对应的 forward_core
        if self.scantype == 'hl':
            y1, y2 = self.forward_core(x)
        elif self.scantype == 'lh':
            y1, y2 = self.forward_core(x)
        elif self.scantype == 'hh':
            y1, y2 = self.forward_core(x)
        else:
            raise ValueError(f"Unsupported scan_type: {self.scantype}")
...
class DSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0))  # (K=3, N, inner)
        del self.x_proj
        
        # 使用 for 循环创建 dt_projs
        self.dt_projs = [
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, **factory_kwargs)
            for _ in range(self.K)
        ]
        self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0))  # (K=3, inner, rank)
        self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0))  # (K=3, inner)
        del self.dt_projs
        
        self.A_logs = self.A_log_init(self.d_state, self.d_inner, copies=3, merge=True)  # (K=3, D, N)
        self.Ds = self.D_init(self.d_inner, copies=3, merge=True)  # (K=3, D, N)

        self.out_norm = nn.LayerNorm(self.d_inner)
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None
        
        self.DS = DeformableLayer(index=stage, embed_dim=self.d_inner, debug=False)
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        y = (out_y[:, 0] + out_y[:, 1].flip(dims=[-1]) + self.DR(out_y[:, 2], indices)) / 3.
