...
from .losses.loss import FFT_Loss, MultiL1Loss
from torch import optim
from torch.nn import functional as F
from .archs.waveelt_utils import _as_wavelet, get_filter_tensors, DWT, IDWT
from torch import nn


class Desmoke_System(Base_Trainer):
    """
    Training system for LoHi-Net with paired data
    """
    def __init__(self,
                 net_desmoke: Dict,
                 loss_name: List[str],
        ...
        super().__init__(predict_save_dir)
    ...
    def training_step(self, batch, batch_idx):
        """
        Training step
        Multi-scale supervision in wavelet domain:
        - out_3 vs gt_2 (1/4 resolution, wavelet domain)
        - out_4 vs gt_1 (1/2 resolution, wavelet domain)
        - out_5 vs real_clear (full resolution)
        """
        smoky, real_clear = batch['lq_files'], batch['gt_files']
        pred_clear = self(smoky)  # [out_3, out_4, out_5]
        
        # Prepare multi-scale ground truth
        # gt_1: 1/2 resolution wavelet-domain features
        ya, (yh, yv, yd) = self.wavedec(real_clear)
        gt_1 = torch.cat([ya, yh, yv, yd], dim=1)
        
        # gt_2: 1/4 resolution wavelet-domain features
        ya, (yh, yv, yd) = self.wavedec(gt_1)
        gt_2 = torch.cat([ya, yh, yv, yd], dim=1)
        ...
        if 'fft_loss' in self.weight_dict:
            fft_loss = self.criterions_fft(pred_clear, [gt_2, gt_1, real_clear])
        ...
        if 'l1_loss' in self.weight_dict or 'l1' in self.weight_dict:
            l1_loss = self.criterions_l1(pred_clear, [gt_2, gt_1, real_clear])
